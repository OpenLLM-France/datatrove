...
from .mathml2latex.mathml2latex import mathml2latex, unicode2latex
...
def improve_latex_content_parsing(html_doc):
    ...
    def extract_latex(elem):
        annotation = elem.query_selector('annotation[encoding="application/x-tex"]')
    ...
    def parse_katex_html(element):
        ...
        for child in element.child_nodes:
            ...
            if "mord" in classes:  # mo character (e.g. F)
                ...
                subscript = subscript.replace("​", "")  # remove the Unicode zero-width space
                if subscript:
                    text += f"_{{{subscript}}}"
            else:
        ...
        def start_callback(context):
            ...
            def replace_sub_sup_tag(node):
                ...
                for child in node.child_nodes:
                    replace_sub_sup_tag(child)  # directly process the child nodes
            ...
            if node.tag == "span" and "intbl" in node.getattr("class", ""):
                ...
                if numerator and denominator:
                    # print("=="*10)
                    node.text = (
                        f"\\frac{{{numerator[0].text}}}{{{denominator[0].text}}}"
                    )
                    # print(node.text)
    ...
    with mem_guard(
        ...
        with time_guard(
            ...
            try:
                ...
                sys.stderr.write("Memory limit exceeded! Returning original HTML.\n")
                sys.stderr.flush()
                return html_doc